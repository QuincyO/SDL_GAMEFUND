...
void Ship::Shoot(std::vector<Bullet*>& container)
{
	...
	{
		const char* FriendBulletPath = "assets/PNG/laserRed.png";
		tempBullet = new Bullet(source, dest, FriendBulletPath);
		container.push_back(new Bullet(tempBullet));
		SoundManager::PlaySound("ShootFX1");
	}
	else if (m_ShipType == ShipType::ENEMY)
	{
		const char* EnemyBulletFilePath = "assets/PNG/laserGreen.png";

		tempBullet = new Bullet(source, dest, EnemyBulletFilePath);
...
void Ship::Update(float deltaTime,std::vector<Bullet*> containerOfBullets)
{
	...
	if (EventManager::KeyPressed(SDL_SCANCODE_SPACE)) shooting = true;
	else shooting = false;
